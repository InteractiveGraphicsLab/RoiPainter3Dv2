...
{
    float px = pivC - texture3D( u_img3_vol, texCd.xyz + u_texCdOfst.xww)+ 0.001;
    float py = pivC - texture3D( u_img3_vol, texCd.xyz + u_texCdOfst.wxw);
    float pz = pivC - texture3D( u_img3_vol, texCd.xyz + u_texCdOfst.wwx);
...
void main(void)
{
    ...
    if( u_doPsuedo != 0 ) gl_FragColor.xyz = texture1D( u_img1_psu, imgI ).xyz;
    else           	      gl_FragColor.xyz = vec3( imgI, imgI, imgI );
	
    vec3 N = calcGradDir( imgI, gl_TexCoord[0].xyz ) ;
    gl_FragColor.xyz = phogShading( worldCoord.xyz, gl_FragColor.xyz, N );
